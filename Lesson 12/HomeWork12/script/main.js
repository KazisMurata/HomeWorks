...
function scrollFunc(n, i) {
  ...
  if (i) {
    ...
    // mainSlider[i].querySelector('.slides').style.left = `${leftIndent}%`;
    
    mainSlidesArea[i].style.left = `${leftIndent}%`;

    for (let j = 0; j < mainDotsArea[i].children.length; j++) {
      mainDotsArea[i].children[j].classList.remove('active');
    }  
    mainDotsArea[i].children[-leftIndent / 100].classList.add('active');
...
function dotScrollFunc(n, k) {
  scrollFunc((leftIndent = -(n * 100 / 2)), k);
  // if (k) {
  //   scrollFunc((leftIndent = -(n * 100 / 2)), k);
  // } else {
...
document.addEventListener('click', function(e) {
  ...
  //       if (e.target === nextBtn[i]) {
  //         scrollFunc(-100, i);
  //       } else if (e.target === prevBtn[i]) {
  //         scrollFunc(100, i);
  //       }
  //     }
  //   }
  ...
  if (e.target.classList.contains('slider-button')) {
    for (let i = 0; i < mainSlider.length; i++) {
      if (e.target === mainSlider[i].querySelector('.next')) {
        scrollFunc(-100, [i]);
      } else if (e.target === mainSlider[i].querySelector('.prev')) {
        scrollFunc(100, [i]);
      }
    }
  }
  ...
  if (e.target.classList.contains('slide-dot')) {
    for (let i = 0; i < mainDotsArea.length; i++) {
      for (let j = 0; j < mainDotsArea[i].children; j++) {
        if (e.target === mainDotsArea[i].children[j]) {
          dotScrollFunc([j], [j]);
        }
      }
    }  
